...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="cca",
        description="ForgeCC — a learning-oriented Python coding agent",
        add_help=False,
    )
...
""")
    ...
    if not resolved_api_key:
        print_error(
            "API key is required.\n"
            "  Set OPENAI_API_KEY (+ optional OPENAI_BASE_URL and OPENAI_MODEL) for OpenAI-compatible format."
        )
        sys.exit(1)
