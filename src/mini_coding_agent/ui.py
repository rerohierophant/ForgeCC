...
def print_cost(input_tokens: int, output_tokens: int, cached_input_tokens: int = 0) -> None:
    uncached_input = max(0, input_tokens - cached_input_tokens)
    discounted_cached_input = cached_input_tokens * 0.10
    cost_in = ((uncached_input + discounted_cached_input) / 1_000_000) * 3
    cost_out = (output_tokens / 1_000_000) * 15
    total = cost_in + cost_out
    cache_info = f" ({cached_input_tokens} cached)" if cached_input_tokens else ""
    console.print(f"\n[dim]  Tokens: {input_tokens} in{cache_info} / {output_tokens} out (~${total:.4f})[/dim]")
